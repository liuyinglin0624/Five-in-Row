...
int CLevelFour::Analysis(CChessBorder* border,int depth,int alpa,int beta)
{
	POINT point;
	point.x = 0;
	point.y = 0;
...
POINTS_CHESS CLevelFour::SeekPoint(CChessBorder* border,int color)
{
...
POINT CLevelFour::TimeToComputer(CChessBorder* border)
{
	Analysis(border,4,INT_MIN,INT_MAX);
	border->BorderArr[des.point.x][des.point.y] = this->nColor;
	return this->des.point;
}
int CLevelFour::EvaluatePoint(CChessBorder* border,POINT point,int color)
{
	//对当前器局进行评估。
	int nVal = 0;
	int opposite;
...
int CLevelFour::GetPoint(CChessBorder* border,POINT point,int FX,int pos)
{
