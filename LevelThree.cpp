...
int CLevelThree::Analysis(CChessBorder* border,int depth,int alpa,int beta)
{
	POINT point;
	point.x = 0;
	point.y = 0;
...
POINTS_CHESS CLevelThree::SeekPoint(CChessBorder* border,int color)
{
...
POINT CLevelThree::TimeToComputer(CChessBorder* border)
{
	Analysis(border,2,INT_MIN,INT_MAX);
	border->BorderArr[des.point.x][des.point.y] = this->nColor;
	return this->des.point;
}
int CLevelThree::EvaluatePoint(CChessBorder* border,POINT point,int color)
{
	//对当前器局进行评估。
	int nVal = 0;
	int opposite;
...
int CLevelThree::GetPoint(CChessBorder* border,POINT point,int FX,int pos)
{
